...
# optional second arg: parallel workers for colcon (e.g. 4)
PARALLEL_WORKERS="${2:-}"
...
if [ $ROS_VERSION = ${VERSION_ROS1} ]; then
    ...
    if [ -n "${PARALLEL_WORKERS}" ]; then
        colcon build --parallel-workers "${PARALLEL_WORKERS}" --cmake-args -DROS_EDITION=${VERSION_ROS2} -DHUMBLE_ROS=${ROS_HUMBLE}
    else
